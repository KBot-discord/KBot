...
export class ReportHandler {
	...
	private async handleCollect(interaction: ButtonInteraction<'cached'>): Promise<void> {
		...
		if (interaction.customId === '@kbotdev/report.timeout') {
		...
		if (interaction.customId === '@kbotdev/report.delete') {
		...
		if (interaction.customId === '@kbotdev/report.info') {
	...
	private async confirmationPrompt(initiator: GuildMember, text: string, type: ReportButtons) {
		const row = new ActionRowBuilder<ButtonBuilder>()
			.addComponents(
				new ButtonBuilder() //
					.setCustomId('@kbotdev/report.confirm')
					.setLabel('Confirm')
					.setStyle(ButtonStyle.Success)
			)
			.addComponents(
				new ButtonBuilder() //
					.setCustomId('@kbotdev/report.cancel')
					.setLabel('Cancel')
		...
		message
			...
			.then(async (i: ButtonInteraction<'cached'>): Promise<void> => {
				...
				if (i.customId === '@kbotdev-report/cancel') {
	...
	private async toggleButton(disabled: boolean, ...buttons: ReportButtons[]): Promise<void> {
		const updatedRow = ActionRowBuilder.from(this.reportMessage.components[0]) as ActionRowBuilder<ButtonBuilder>;
