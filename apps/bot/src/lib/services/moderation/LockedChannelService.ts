...
export class LockedChannelService {
	...
	public async getChannelMessagePermissions(channel: GuildTextBasedChannel, roleId: string): Promise<boolean | undefined> {
		...
		if (channel.isThread()) {
			if (isNullish(channel.parent)) {
				...
				const permissionsForRole = parent.permissionsFor(roleId);
				result = permissionsForRole?.has(PermissionFlagsBits.SendMessagesInThreads);
			} else {
				const permissionsForRole = channel.parent.permissionsFor(roleId);
				result = permissionsForRole?.has(PermissionFlagsBits.SendMessagesInThreads);
			}
		} else {
			const permissionsForRole = channel.permissionsFor(roleId);
			result = permissionsForRole?.has(PermissionFlagsBits.SendMessagesInThreads);
		}
