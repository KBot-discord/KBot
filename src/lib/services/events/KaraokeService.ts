...
export class KaraokeService {
	...
	public async endEvent(guild: Guild, event: KaraokeEvent) {
		...
		await this.setEventExists({ eventId: event.id, guildId: event.guildId }, false);
		await this.setEventActive({ eventId: event.id, guildId: event.guildId }, false);
		await this.deleteEvent({ eventId: event.id });
	}
