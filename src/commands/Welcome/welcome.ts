...
import { ChannelType, channelMention, EmbedBuilder, PermissionFlagsBits } from 'discord.js';
import { KBotSubcommand } from '../../lib/extensions/KBotSubcommand.js';
import { KBotModules } from '../../lib/types/Enums.js';
import { EmbedColors, HexColorRegex, KBotEmoji } from '../../lib/utilities/constants.js';
...
export class EventsCommand extends KBotSubcommand<WelcomeModule> {
	...
	public async chatInputSet(interaction: KBotSubcommand.ChatInputCommandInteraction): Promise<unknown> {
		...
		if (!isNullOrUndefined(color) && HexColorRegex.test(color)) {
			return await interaction.errorReply('Please provide a valid Hex color.');
		}
	...
	public async chatInputTest(interaction: KBotSubcommand.ChatInputCommandInteraction): Promise<unknown> {
		...
		if (!settings || (!settings.message && !settings.title && !settings.description && !settings.image)) {
			return await interaction.defaultReply('There are no settings to test');
		}
		...
		const fields: APIEmbedField[] = [
			{
				...
				value: settings?.enabled
					? ':white_check_mark: Welcome messages are enabled'
					: ':x: Welcome messages are disabled',
			},
			{
				name: 'Channel:',
