...
export class ButtonHandler extends InteractionHandler {
	private readonly customIds = [KaraokeCustomIds.Add];

	public override async run(interaction: ButtonInteraction, {}: InteractionHandler.ParseResult<this>) {
		try {
			return interaction.errorFollowup('Add to queue.', true);
		} catch (err) {
			this.container.logger.error(err);
			return interaction.errorFollowup('There was an error when trying to add you to the queue.', true);
		}
	}
