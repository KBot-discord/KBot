...
export class EventsCommand extends KBotSubcommand<EventModule> {
	...
	public override async autocompleteRun(interaction: KBotSubcommand.AutocompleteInteraction): Promise<void> {
		...
		if (focusedOption.name === 'event') {
			...
			const channelData = await Promise.all(events.map(async ({ id }) => await fetchChannel(id)));
			...
			await interaction.respond(eventOptions);
			return;
		}
		...
		await interaction.respond(discordEventOptions);
	}
	...
	public async chatInputKaraokeStart(interaction: KBotSubcommand.ChatInputCommandInteraction): Promise<unknown> {
		...
		if (event) {
			return await interaction.defaultReply('There is already an event in that channel.');
		}
		...
		return await result.match({
			ok: async () => {
				return await interaction.successReply('The karaoke event has started.');
			},
			err: async (error) => {
	...
	public async chatInputKaraokeSchedule(interaction: KBotSubcommand.ChatInputCommandInteraction): Promise<unknown> {
		...
		if (isNullOrUndefined(discordEvent.channelId) || isNullOrUndefined(discordEvent.channel)) {
			return await interaction.defaultReply('There is no channel set for that Discord event.');
		}

		const voiceChannel = await fetchChannel<VoiceBasedChannel>(discordEvent.channelId);
		const voiceResult = await validator.channels.canModerateVoice(voiceChannel);
		if (!voiceResult.result) {
			return interaction.client.emit(KBotErrors.ChannelPermissions, {
				interaction,
	...
	public async chatInputKaraokeStop(interaction: KBotSubcommand.ChatInputCommandInteraction): Promise<unknown> {
		...
		if (!event) {
			return await interaction.errorReply('There is no event with the ID you provided.');
		}

		if (!event.isActive) {
			return await interaction.defaultReply('There is no karaoke event to stop.');
		}

		const result = await this.module.karaoke.endEvent(event);

		return await result.match({
			ok: async () => {
				return await interaction.successReply('The karaoke event has ended.');
			},
			err: async (error) => {
				this.container.logger.sentryError(error, {
					context: {
	...
	public async chatInputKaraokeAdd(interaction: KBotSubcommand.ChatInputCommandInteraction): Promise<unknown> {
		...
		if (isNullOrUndefined(member)) {
			return await interaction.errorReply('That user is not in this server.');
		}

		if (isNullOrUndefined(member.voice.channel) || member.voice.channelId !== eventId) {
			return await interaction.errorReply('That user is not in the event channel.');
		}

		const event = await karaoke.getEventWithQueue(eventId);
		if (isNullOrUndefined(event) || !event.isActive) {
			return await interaction.defaultReply('There is no karaoke event to join.');
		}
		...
		if (!valid) {
			return await interaction.errorReply(reason);
		}
		...
		return await interaction.successReply(`${member.user.username} has been added to the queue.`);
	}
	...
	public async chatInputKaraokeRemove(interaction: KBotSubcommand.ChatInputCommandInteraction): Promise<unknown> {
		...
		if (isNullOrUndefined(userEntry)) {
			return await interaction.defaultReply('That user is not in the queue.');
		}
		...
		return await interaction.successReply(`${member.user.username} has been removed from the queue.`);
	}

	public async chatInputKaraokeMenu(interaction: KBotSubcommand.ChatInputCommandInteraction): Promise<unknown> {
		return await new KaraokeEventMenu(interaction.guild).run(interaction);
	}
}
