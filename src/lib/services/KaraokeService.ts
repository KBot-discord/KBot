...
export class KaraokeService extends ResultClass {
	...
	public async createEvent(data: {
		...
		return await container.prisma.karaokeEvent.create({
			data: {
				id,
				textChannelId,
				locked: false,
	 ...
	 */
	public async createScheduledEvent(data: {
		id: string;
		textChannelId: string;
	...
	public async addUserToQueue(
		...
		const result = await container.prisma.karaokeUser.create({
	...
	public async removeUserFromQueue(
		...
		const result = await container.prisma.karaokeUser.delete({
